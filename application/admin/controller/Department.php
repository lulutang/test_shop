...
class Department
{
    		...
    		foreach ($list as &$val){
    			...
    			if($lists){
    				...
    				foreach ($lists as &$va){
    					...
    					$where = array('status' => 0, 'pid'=>$va['id']);
    					$lists_san = Db::name("ViewAdminDepartment")->where($where)->order('id desc')->select();
    					if($lists_san){
    						$va['sanji'] = $lists_san;
   ...
   public function delete()
   {
   	if ($this->request->isAjax() && $this->request->isPost()) {
   		...
   		$ids= explode(',', $data['id']);
   		Db::name("ViewAdminDepartment")->where(array('pid' => array('in',$data['id'])))->update($log);
